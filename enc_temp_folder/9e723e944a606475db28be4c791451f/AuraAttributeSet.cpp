...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	/** Description 
	 * Armor(Res) : Reduces damage taken, improves Block Chance
	 * Armor Penetration(Res) : Ignores percentage of enemy Armor, increases Crit Hit Chance
	 * Block Chance(Armor) : Chance to cut incoming damage in half
	 * Critical Hit Chance(Armor Penetration) : Chance to double damage plus critical hit bonus
	 * Critical Hit Damage(Armor Penetration) : Bonus damage added when a critical hit is scored
	 * Critical Hit Resistance (Armor) : Reduces critical hit chance of attacking enemies
	 * Health Regeneratrion(Vig) : Amount of Health regenerated every 1 second
	 * Mana Regeneration(Int) : Amount of Mana regenerated every 1 second
	 * Max Health (Vig) : Maximum amount of Health obtainable
	 * Max Mana (Int) : Maximum amount of Mana obtainable
	 */
...
void UAuraAttributeSet::PreAttributeChange(const FGameplayAttribute &Attribute, float &NewValue)
{
	 ...
	 *    1) before the change happens
	 * 
	 *   - Triggered by changes to Attributes
	 *    1) Attribute Accessors
	 *    2) Gameplay Effects
	 * 
	 *   - Does note permanently change the modifier, just the value returned from querying the modifier
	 *   - Later operations recalculate the Current Value from all modifiers
	 *    1) We need to clamp again
	 */
}
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		{
			if (Props.TargetCharacter->Implements<UCombatInterface>() && !ICombatInterface::Execute_IsBeingShockLoop(Props.TargetCharacter))
			{
